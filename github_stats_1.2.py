...
from gh_api import get_paged_request, make_auth_header, get_pull_request, api_request
...
def has_label(issue, label):
    return any(lab['name'] == label for lab in issue['labels'])


def get_user(pr):
    user = api_request(pr['user']['url'], auth)
    return user['name']
...
def issues_closed_since(period=timedelta(days=365), project="ipython/ipython", pulls=False):
    ...
    if pulls:
        filtered = [ i for i in filtered if is_pull_request(i) ]
    ...
    filtered = [ i for i in filtered if  has_label(i, 'backport-1.2') ]
    return filtered
...
if __name__ == "__main__":
    ...
    authors = []
    for pr in pulls:
        authors.extend(get_authors(pr))
    ncommits = len(authors) + ncommits - len(pulls)
    print(authors)
    author_cmd = ['git', 'check-mailmap'] + authors
    all_authors = check_output(author_cmd).decode('utf-8', 'replace').splitlines()
    all_authors = [ u'* ' + a.split(' <')[0] for a in all_authors ]
    unique_authors = sorted(set(all_authors), key=lambda s: s.lower())
